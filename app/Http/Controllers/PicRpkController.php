    ...
    public function uploadrpk(Request $request){
        ...
        if(Auth::user()->cabang == 'Banjarmasin' or Auth::user()->cabang == 'Bunati'){
            ...
            if ($request->hasFile('bjrfile1')) {
                ...
                if(documentrpk::where('nama_kapal', 'Like', '%' . $request->nama_kapal . '%')->where('cabang' , Auth::user()->cabang)->whereColumn('created_at' , '<=', 'periode_akhir')->exists()){
                    
                    documentrpk::where('nama_kapal', 'Like', '%' . $request->nama_kapal . '%')->where('cabang' , Auth::user()->cabang)->whereColumn('created_at' , '<=', 'periode_akhir')->update([
                        'status1' => 'on review',
                        'surat_barang' => basename($path),
                        'time_upload1' => date("Y-m-d h:i:s"),
                    ]);
