...
class DashboardController extends Controller
{
    public function index(){
        if(Auth::user()->hasRole('crew')){
            ...
            // Get all the order
            $orderHeads = OrderHead::where('status', 'like', '%In Progress%', 'or','created_at', '>=', Carbon::now()->subDays(30))->paginate(10);

            // Get all the order detail
            $order_id = OrderHead::select('order_id')->where('status', 'like', '%In Progress%', 'or','created_at', '>=', Carbon::now()->subDays(30))->pluck('order_id');
            $orderDetails = OrderDetail::whereIn('orders_id', $order_id)->join('items', 'items.itemName', '=', 'order_details.itemName')->get();
