...
class PurchasingReportExport implements FromQuery, WithHeadings, ShouldAutoSize, WithEvents
{
    public function query()
    {
        ...
        return OrderHead::join('suppliers', 'suppliers.id', '=', 'order_heads.supplier_id')->whereIn('user_id', $users)->where('status', 'like', 'Order Completed (Logistic)')->whereMonth('order_heads.created_at', date('m'))->whereYear('order_heads.created_at', date('Y'))->where('cabang', 'like', Auth::user()->cabang)->select('prDate', 'noPr', 'suppliers.supplierName', 'noPo', 'boatName', 'descriptions')->orderBy('order_heads.updated_at', 'desc');
    }
